...
if [ -f "$KEY_FILE" ]; then
    ...
    echo "WARNING: No service-account key mounted or provided via SERVICE_ACCOUNT_JSON_B64"
fi
...
if [ -f "$CONFIG_FILE" ]; then
    ...
    echo "WARNING: No app config mounted or provided via APP_CONFIG_JSON_B64"
fi
...
if [ -f "$RCLONE_FILE" ]; then
    ...
    echo "WARNING: No rclone config mounted or provided via RCLONE_CONF_B64"
fi
