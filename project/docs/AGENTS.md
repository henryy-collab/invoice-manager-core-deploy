...
5. Write info to Report appends processed invoice details (Client Ref., PDF Invoice Date, PDF Invoice No., Topped amount) to the configured Google Sheets spreadsheet, in a protected tab named for the invoice month/year with an `[Auto]` suffix (e.g. `Apr 2026 [Auto]`). To add your own columns or formatting, duplicate the [Auto] tab in Google Sheets.
