...
- **`GET /api/accounts`** is a separate endpoint (independent of the parse/preview/report flow) that returns an aggregated, account-keyed data array from the archived processed PDFs (`config.archive_folder`, optionally overridden with `?folder=`). It lives in `ui/invoice_ui/services/accounts_service.py` and `ui/invoice_ui/routers/accounts_router.py`.
